...
import { Activity, ArrowDownRight, ArrowUpRight, Building2, CalendarRange, Check, ChevronRight, CircleDollarSign, ClipboardList, Home, LayoutDashboard, LogOut, Menu, MoreHorizontal, Pencil, Plus, ReceiptIndianRupee, Search, Sparkles, Trash2, UserCog, WalletCards, X } from 'lucide-react';
...
function AppShell({ owner }: { owner: Owner }) {
  ...
  const [flatForm, setFlatForm] = useState({ propertyId: '', flatNo: '', tenantName: '', workplace: '', govtId: '', moveInDate: '', tenureEnd: '', deposit: '', rent: '' });
  ...
  const submitProperty = (event: FormEvent<HTMLFormElement>) => {
    ...
    const onSuccess = () => {
      ...
      refreshOverview();
      setNotice({ tone: 'success', text: editingProperty ? 'Property details updated.' : 'Property added to your workspace.' });
    };
    if (editingProperty) {
      updateProperty.mutate({ id: editingProperty.id, data: propertyForm }, { onSuccess });
    } else {
      createProperty.mutate({ data: propertyForm }, { onSuccess });
    }
  };
  ...
  const openCreateFlat = (propertyId = properties[0]?.id ?? '') => {
    setEditingFlat(null);
    setFlatForm({ propertyId, flatNo: '', tenantName: '', workplace: '', govtId: '', moveInDate: '', tenureEnd: '', deposit: '', rent: '' });
    setFlatModal(true);
  };

  const openEditFlat = (flat: Flat) => {
    setEditingFlat(flat);
    setFlatForm({ propertyId: flat.propertyId, flatNo: flat.flatNo, tenantName: flat.tenantName, workplace: flat.workplace, govtId: flat.govtId, moveInDate: flat.moveInDate.slice(0, 10), tenureEnd: flat.tenureEnd.slice(0, 10), deposit: String(flat.deposit), rent: String(flat.rent) });
    setFlatModal(true);
  };

  const openResolveExpired = (flat: Flat) => {
    setExpiredFlat(flat);
    setRenewForm({ tenureEnd: '', rent: String(flat.rent) });
  };
  ...
  const submitFlat = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!flatForm.propertyId || !flatForm.flatNo.trim() || !flatForm.tenantName.trim() || !flatForm.rent || !flatForm.moveInDate || !flatForm.tenureEnd) return;
    const flatData = { flatNo: flatForm.flatNo, tenantName: flatForm.tenantName, workplace: flatForm.workplace, govtId: flatForm.govtId, moveInDate: flatForm.moveInDate, tenureEnd: flatForm.tenureEnd, deposit: Number(flatForm.deposit) || 0, rent: Number(flatForm.rent) || 0 };
    const onSuccess = () => {
      setFlatModal(false);
      refreshOverview();
      setNotice({ tone: 'success', text: editingFlat ? 'Unit details updated.' : 'Unit and tenant added.' });
    };
    if (editingFlat) {
      updateFlat.mutate({ id: editingFlat.id, data: flatData }, { onSuccess });
    } else {
      createFlat.mutate({ data: { propertyId: flatForm.propertyId, ...flatData } }, { onSuccess });
    }
  };

  const removeFlat = (flat: Flat) => {
    if (!window.confirm(`Remove unit ${flat.flatNo}?`)) return;
  ...
  const openPayment = (flat: Flat) => {
    ...
    setPaymentForm({ paymentDate: new Date().toISOString().slice(0, 10), amount: String(flat.rent) });
  ...
  const submitPayment = (event: FormEvent<HTMLFormElement>) => {
    ...
    createPayment.mutate({ id: selectedFlat.id, data: { paymentDate: paymentForm.paymentDate, amount: Number(paymentForm.amount) } }, {
      onSuccess: () => {
        ...
        setNotice({ tone: 'success', text: `Payment recorded for ${selectedFlat.tenantName}.` });
      },
    ...
    <div className="noise flex min-h-[100dvh] bg-[hsl(var(--background))]">
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="workspace-grid mx-auto min-h-[calc(100dvh-70px)] max-w-[1600px] px-5 py-8 sm:px-8 lg:px-12">
          ...
          {isAnyError ? <ErrorState onRetry={retryQueries} /> : isInitialLoading ? <WorkspaceSkeleton /> : view === 'overview' ? <Overview properties={properties} flats={flats} summary={summary} activities={activities} onAddProperty={openCreateProperty} onEditProperty={openEditProperty} onDeleteProperty={removeProperty} onAddFlat={openCreateFlat} onSelectFlat={setSelectedFlat} /> : <Ledger flats={filteredFlats} allFlats={flats} properties={properties} search={ledgerSearch} onSearch={setLedgerSearch} propertyFilter={propertyFilter} onPropertyFilter={setPropertyFilter} onAddFlat={openCreateFlat} onEditFlat={openEditFlat} onDeleteFlat={removeFlat} onRecordPayment={openPayment} onSelectFlat={setSelectedFlat} onResolveExpired={openResolveExpired} />}
        </div>
      </main>
      ...
      <ExpiredLeaseModal flat={expiredFlat} form={renewForm} setForm={setRenewForm} onClose={() => setExpiredFlat(null)} onSubmitRenew={submitRenew} onDelete={() => expiredFlat && removeFlat(expiredFlat)} renewPending={renewFlat.isPending} />
      <ProfileModal open={profileModal} owner={owner} onClose={() => setProfileModal(false)} onNotice={setNotice} />
      {selectedFlat && <FlatDetail flat={selectedFlat} payments={paymentsQuery.data ?? []} loading={paymentsQuery.isLoading} onClose={() => setSelectedFlat(null)} onEdit={() => openEditFlat(selectedFlat)} onRecordPayment={() => openPayment(selectedFlat)} onResolveExpired={() => openResolveExpired(selectedFlat)} />}
    </div>
  );
}
...
function PropertyCard({ property, flats, onEdit, onDelete, onAddFlat, onSelectFlat }: { property: Property; flats: Flat[]; onEdit: () => void; onDelete: () => void; onAddFlat: () => void; onSelectFlat: (flat: Flat) => void }) {
  const collected = flats.reduce((sum, flat) => sum + flat.totalPaid, 0);
  return <Surface className="group overflow-hidden transition-transform duration-300 hover:-translate-y-1"><div className="border-b border-[hsl(var(--border))] bg-[hsl(var(--sidebar))] px-5 pb-5 pt-4 text-[hsl(var(--sidebar-foreground))]"><div className="mb-5 flex items-start justify-between"><span className="mono rounded-full bg-[hsl(var(--sidebar-primary)/.16)] px-2 py-1 text-[9px] uppercase tracking-[.12em] text-[hsl(var(--sidebar-primary))]">Property</span><div className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100"><button type="button" onClick={onEdit} data-testid={`button-edit-property-${property.id}`} className="rounded-md p-1.5 text-[hsl(var(--sidebar-foreground)/.65)] hover:bg-[hsl(var(--sidebar-accent))] hover:text-[hsl(var(--sidebar-foreground))]"><Pencil size={13} /></button><button type="button" onClick={onDelete} data-testid={`button-delete-property-${property.id}`} className="rounded-md p-1.5 text-[hsl(var(--sidebar-foreground)/.65)] hover:bg-[hsl(var(--destructive)/.2)] hover:text-[hsl(var(--destructive))]"><Trash2 size={13} /></button></div></div><h3 className="display truncate text-[17px] font-semibold tracking-[-.04em]">{property.name}</h3><p className="mt-1 flex items-center gap-1.5 truncate text-[11px] text-[hsl(var(--sidebar-foreground)/.56)]"><Building2 size={12} />{property.address}</p></div><div className="p-4"><div className="mb-3 grid grid-cols-3 divide-x divide-[hsl(var(--border))]"><div><p className="mono text-[9px] uppercase text-[hsl(var(--muted-foreground))]">Units</p><p className="mt-1 text-[15px] font-bold">{flats.length}<span className="font-normal text-[10px] text-[hsl(var(--muted-foreground))]"> / {property.unitCount}</span></p></div><div className="pl-3"><p className="mono text-[9px] uppercase text-[hsl(var(--muted-foreground))]">Occupancy</p><p className="mt-1 text-[15px] font-bold">{property.unitCount ? Math.round(flats.length / property.unitCount * 100) : 0}%</p></div><div className="pl-3"><p className="mono text-[9px] uppercase text-[hsl(var(--muted-foreground))]">Collected</p><p className="mt-1 truncate text-[15px] font-bold">{formatMoney(collected)}</p></div></div>{flats.length ? <div className="mb-3 flex flex-wrap gap-1.5">{flats.slice(0, 3).map((flat) => <button key={flat.id} type="button" onClick={() => onSelectFlat(flat)} data-testid={`button-flat-${flat.id}`} className="rounded-md border border-[hsl(var(--border))] px-2 py-1 text-[10px] font-semibold transition-colors hover:border-[hsl(var(--primary))]">{flat.flatNo} · {flat.tenantName.split(' ')[0]}</button>)}{flats.length > 3 && <span className="px-1 py-1 text-[10px] text-[hsl(var(--muted-foreground))]">+{flats.length - 3} more</span>}</div> : <p className="mb-3 text-[11px] text-[hsl(var(--muted-foreground))]">No units assigned yet.</p>}<button type="button" onClick={onAddFlat} data-testid={`button-add-flat-${property.id}`} className="flex w-full items-center justify-center gap-1.5 rounded-[8px] border border-dashed border-[hsl(var(--border))] py-2 text-[11px] font-bold text-[hsl(var(--muted-foreground))] transition-colors hover:border-[hsl(var(--primary))] hover:bg-[hsl(var(--primary)/.06)] hover:text-[hsl(var(--primary-foreground))]"><Plus size={13} />Add unit</button></div></Surface>;
}

function Ledger({ flats, allFlats, properties, search, onSearch, propertyFilter, onPropertyFilter, onAddFlat, onEditFlat, onDeleteFlat, onRecordPayment, onSelectFlat, onResolveExpired }: { flats: Flat[]; allFlats: Flat[]; properties: Property[]; search: string; onSearch: (value: string) => void; propertyFilter: string; onPropertyFilter: (value: string) => void; onAddFlat: () => void; onEditFlat: (flat: Flat) => void; onDeleteFlat: (flat: Flat) => void; onRecordPayment: (flat: Flat) => void; onSelectFlat: (flat: Flat) => void; onResolveExpired: (flat: Flat) => void }) {
  return <div className="animate-rise-in space-y-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mono mb-1 text-[10px] uppercase tracking-[.18em] text-[hsl(var(--muted-foreground))]">Collections / live view</p><h2 className="display text-[26px] font-semibold tracking-[-.055em]">Rent ledger</h2><p className="mt-1 text-[13px] text-[hsl(var(--muted-foreground))]">{allFlats.length} active unit{allFlats.length === 1 ? '' : 's'} across {properties.length} propert{properties.length === 1 ? 'y' : 'ies'}.</p></div><button type="button" onClick={onAddFlat} data-testid="button-ledger-add-flat" className="flex items-center justify-center gap-2 rounded-[9px] bg-[hsl(var(--primary))] px-3.5 py-2.5 text-[12px] font-bold text-[hsl(var(--primary-foreground))] transition-transform hover:-translate-y-0.5"><Plus size={15} />Add unit</button></div><Surface className="overflow-hidden"><div className="flex flex-col gap-3 border-b border-[hsl(var(--border))] bg-[hsl(var(--muted)/.3)] p-4 sm:flex-row"><div className="relative flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" /><input type="search" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search tenant, unit or property" data-testid="input-ledger-search" className={`${inputClass()} pl-9`} /></div><select value={propertyFilter} onChange={(event) => onPropertyFilter(event.target.value)} data-testid="select-ledger-property" className={`${inputClass()} sm:w-[190px]`}><option value="all">All properties</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.name}</option>)}</select></div>{flats.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr className="border-b border-[hsl(var(--border))] text-[10px] uppercase tracking-[.12em] text-[hsl(var(--muted-foreground))]"><th className="px-5 py-3 font-semibold">Unit / tenant</th><th className="px-3 py-3 font-semibold">Monthly rent</th><th className="px-3 py-3 font-semibold">Collected</th><th className="px-3 py-3 font-semibold">Last payment</th><th className="px-3 py-3 font-semibold">Status</th><th className="px-5 py-3 text-right font-semibold">Actions</th></tr></thead><tbody>{flats.map((flat) => <LedgerRow key={flat.id} flat={flat} onEdit={() => onEditFlat(flat)} onDelete={() => onDeleteFlat(flat)} onPay={() => onRecordPayment(flat)} onSelect={() => onSelectFlat(flat)} onResolve={() => onResolveExpired(flat)} />)}</tbody></table></div> : <div className="p-5"><EmptyState title={allFlats.length ? 'No matching units' : 'The ledger is ready'} description={allFlats.length ? 'Try a different search or property filter.' : 'Add a unit to begin tracking rent and payment history.'} action={!allFlats.length ? <button type="button" onClick={onAddFlat} data-testid="button-empty-ledger-add-flat" className="rounded-[9px] bg-[hsl(var(--primary))] px-4 py-2.5 text-[12px] font-bold text-[hsl(var(--primary-foreground))]"><Plus size={14} className="mr-1.5 inline" />Add first unit</button> : undefined} /></div>}</Surface></div>;
}

function LedgerRow({ flat, onEdit, onDelete, onPay, onSelect, onResolve }: { flat: Flat; onEdit: () => void; onDelete: () => void; onPay: () => void; onSelect: () => void; onResolve: () => void }) {
  const paidThisCycle = flat.lastPaymentDate && new Date(flat.lastPaymentDate).getMonth() === new Date().getMonth();
  return <tr data-testid={`row-ledger-${flat.id}`} className="group border-b border-[hsl(var(--border)/.7)] last:border-0 transition-colors hover:bg-[hsl(var(--muted)/.38)]"><td className="px-5 py-4"><button type="button" onClick={onSelect} data-testid={`button-open-flat-${flat.id}`} className="flex items-center gap-3 text-left"><span className="flex h-9 w-9 items-center justify-center rounded-[9px] bg-[hsl(var(--sidebar))] text-[11px] font-bold text-[hsl(var(--sidebar-primary))]">{flat.flatNo.slice(-3)}</span><span><span className="block text-[12px] font-bold">{flat.tenantName}</span><span className="mt-0.5 block text-[10px] text-[hsl(var(--muted-foreground))]">{flat.propertyName} · {flat.flatNo}</span></span></button></td><td className="px-3 py-4 mono text-[12px] font-medium">{formatMoney(flat.rent)}</td><td className="px-3 py-4 mono text-[12px] font-medium">{formatMoney(flat.totalPaid)}</td><td className="px-3 py-4 text-[11px] text-[hsl(var(--muted-foreground))]">{formatDate(flat.lastPaymentDate, true)}</td><td className="px-3 py-4">{flat.isExpired ? <span className="inline-flex items-center gap-1.5 rounded-full bg-[hsl(var(--destructive)/.14)] px-2 py-1 text-[10px] font-bold text-[hsl(var(--destructive))]"><span className="h-1.5 w-1.5 rounded-full bg-current" />Expired</span> : <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[10px] font-bold ${paidThisCycle ? 'bg-[hsl(var(--primary)/.16)] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(var(--accent)/.16)] text-[hsl(var(--accent-foreground))]'}`}><span className="h-1.5 w-1.5 rounded-full bg-current" />{paidThisCycle ? 'Paid' : 'Due'}</span>}</td><td className="px-5 py-4"><div className="flex justify-end gap-1">{flat.isExpired ? <button type="button" onClick={onResolve} data-testid={`button-resolve-flat-${flat.id}`} className="rounded-[7px] bg-[hsl(var(--destructive)/.13)] px-2.5 py-1.5 text-[10px] font-bold text-[hsl(var(--destructive))] transition-colors hover:bg-[hsl(var(--destructive)/.22)]">Resolve</button> : <button type="button" onClick={onPay} data-testid={`button-record-payment-${flat.id}`} className="rounded-[7px] bg-[hsl(var(--primary)/.13)] px-2.5 py-1.5 text-[10px] font-bold text-[hsl(var(--primary-foreground))] transition-colors hover:bg-[hsl(var(--primary)/.24)]">Record payment</button>}<button type="button" onClick={onEdit} data-testid={`button-edit-flat-${flat.id}`} className="rounded-md p-1.5 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--foreground))]"><Pencil size={14} /></button><button type="button" onClick={onDelete} data-testid={`button-delete-flat-${flat.id}`} className="rounded-md p-1.5 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--destructive)/.1)] hover:text-[hsl(var(--destructive))]"><Trash2 size={14} /></button></div></td></tr>;
}

function FlatDetail({ flat, payments, loading, onClose, onEdit, onRecordPayment, onResolveExpired }: { flat: Flat; payments: { id: string; paymentDate: string; amount: number }[]; loading: boolean; onClose: () => void; onEdit: () => void; onRecordPayment: () => void; onResolveExpired: () => void }) {
  return <div className="fixed inset-0 z-30 flex justify-end bg-[hsl(var(--foreground)/.22)] backdrop-blur-[1px]"><section className="animate-rise-in h-full w-full max-w-[420px] overflow-y-auto border-l border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 shadow-[-20px_0_60px_hsl(var(--foreground)/.12)] sm:p-7"><div className="mb-8 flex items-start justify-between"><div><p className="mono text-[10px] uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Unit detail</p><h2 className="display mt-1 text-[24px] font-semibold tracking-[-.05em]">{flat.flatNo}</h2></div><button type="button" onClick={onClose} data-testid="button-close-flat-detail" className="rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]"><X size={17} /></button></div>{flat.isExpired && <div data-testid="badge-detail-expired" className="mb-4 flex items-center gap-2 rounded-[9px] bg-[hsl(var(--destructive)/.12)] px-3.5 py-2.5 text-[11px] font-bold text-[hsl(var(--destructive))]"><span className="h-1.5 w-1.5 rounded-full bg-current" />Lease expired {formatDate(flat.tenureEnd, true)}</div>}<div className="rounded-[13px] bg-[hsl(var(--sidebar))] p-5 text-[hsl(var(--sidebar-foreground))]"><p className="text-[16px] font-bold">{flat.tenantName}</p><p className="mt-1 text-[11px] text-[hsl(var(--sidebar-foreground)/.58)]">{flat.propertyName} · {flat.workplace || 'Workplace not added'}</p><div className="mt-6 grid grid-cols-2 gap-4"><div><p className="mono text-[9px] uppercase text-[hsl(var(--sidebar-foreground)/.46)]">Monthly rent</p><p className="mt-1 display text-[20px] font-semibold text-[hsl(var(--sidebar-primary))]">{formatMoney(flat.rent)}</p></div><div><p className="mono text-[9px] uppercase text-[hsl(var(--sidebar-foreground)/.46)]">Total paid</p><p className="mt-1 display text-[20px] font-semibold">{formatMoney(flat.totalPaid)}</p></div></div></div><div className="mt-4 flex gap-2">{flat.isExpired ? <button type="button" onClick={onResolveExpired} data-testid="button-detail-resolve" className="flex flex-1 items-center justify-center gap-1.5 rounded-[8px] bg-[hsl(var(--destructive))] py-2.5 text-[11px] font-bold text-[hsl(var(--destructive-foreground))]">Resolve lease</button> : <button type="button" onClick={onRecordPayment} data-testid="button-detail-record-payment" className="flex flex-1 items-center justify-center gap-1.5 rounded-[8px] bg-[hsl(var(--primary))] py-2.5 text-[11px] font-bold text-[hsl(var(--primary-foreground))]"><Plus size={14} />Record payment</button>}<button type="button" onClick={onEdit} data-testid="button-detail-edit-flat" className="flex items-center justify-center gap-1.5 rounded-[8px] border border-[hsl(var(--border))] px-3 py-2.5 text-[11px] font-bold"><Pencil size={14} />Edit</button></div><div className="mt-8"><SectionTitle eyebrow="Payment history" title="Recent payments" />{loading ? <SkeletonRows rows={3} /> : payments.length ? <div className="space-y-2">{payments.map((payment) => <div key={payment.id} data-testid={`payment-${payment.id}`} className="flex items-center justify-between rounded-[9px] border border-[hsl(var(--border))] px-3.5 py-3"><div className="flex items-center gap-2.5"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[hsl(var(--primary)/.14)] text-[hsl(var(--primary-foreground))]"><CircleDollarSign size={14} /></span><span className="text-[11px] font-semibold">{formatDate(payment.paymentDate, true)}</span></div><span className="mono text-[12px] font-medium">{formatMoney(payment.amount)}</span></div>)}</div> : <EmptyState title="No payments yet" description="Record the first rent payment for this unit." />}</div><div className="mt-8 border-t border-[hsl(var(--border))] pt-5"><div className="flex justify-between text-[11px]"><span className="text-[hsl(var(--muted-foreground))]">Move-in date</span><span className="font-semibold">{formatDate(flat.moveInDate, true)}</span></div><div className="mt-3 flex justify-between text-[11px]"><span className="text-[hsl(var(--muted-foreground))]">Tenure ends</span><span className={`font-semibold ${flat.isExpired ? 'text-[hsl(var(--destructive))]' : ''}`}>{formatDate(flat.tenureEnd, true)}</span></div><div className="mt-3 flex justify-between text-[11px]"><span className="text-[hsl(var(--muted-foreground))]">Security deposit</span><span className="mono font-medium">{formatMoney(flat.deposit)}</span></div></div></section></div>;
}

function PropertyModal({ open, editing, form, setForm, onClose, onSubmit, pending }: { open: boolean; editing: Property | null; form: { name: string; address: string }; setForm: (form: { name: string; address: string }) => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; pending: boolean }) {
  return <Modal open={open} title={editing ? 'Edit property' : 'Add a property'} description="A clear name and address is all you need to begin." onClose={onClose}><form onSubmit={onSubmit} className="space-y-4"><Field label="Property name"><input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} data-testid="input-property-name" className={inputClass()} placeholder="e.g. Palm Grove Apartments" /></Field><Field label="Address"><input required value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} data-testid="input-property-address" className={inputClass()} placeholder="Street, locality, city" /></Field><div className="flex justify-end gap-2 pt-3"><button type="button" onClick={onClose} data-testid="button-cancel-property" className="rounded-[8px] px-3.5 py-2.5 text-[12px] font-semibold text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]">Cancel</button><button type="submit" disabled={pending} data-testid="button-submit-property" className="rounded-[8px] bg-[hsl(var(--primary))] px-4 py-2.5 text-[12px] font-bold text-[hsl(var(--primary-foreground))] disabled:opacity-50">{pending ? 'Saving...' : editing ? 'Save changes' : 'Add property'}</button></div></form></Modal>;
}
...
function FlatModal({ open, editing, form, setForm, properties, onClose, onSubmit, pending }: { open: boolean; editing: Flat | null; form: { propertyId: string; flatNo: string; tenantName: string; workplace: string; govtId: string; moveInDate: string; tenureEnd: string; deposit: string; rent: string }; setForm: (form: { propertyId: string; flatNo: string; tenantName: string; workplace: string; govtId: string; moveInDate: string; tenureEnd: string; deposit: string; rent: string }) => void; properties: Property[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; pending: boolean }) {
  return <Modal open={open} title={editing ? 'Edit unit' : 'Add a unit'} description="Keep the lease essentials close. You can refine details later." onClose={onClose}><form onSubmit={onSubmit} className="space-y-4"><Field label="Property"><select required disabled={!!editing} value={form.propertyId} onChange={(event) => setForm({ ...form, propertyId: event.target.value })} data-testid="select-flat-property" className={inputClass()}><option value="">Choose a property</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.name}</option>)}</select></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Unit number"><input required value={form.flatNo} onChange={(event) => setForm({ ...form, flatNo: event.target.value })} data-testid="input-flat-number" className={inputClass()} placeholder="A-204" /></Field><Field label="Tenant name"><input required value={form.tenantName} onChange={(event) => setForm({ ...form, tenantName: event.target.value })} data-testid="input-tenant-name" className={inputClass()} placeholder="Tenant's full name" /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Monthly rent"><input required min="0" type="number" value={form.rent} onChange={(event) => setForm({ ...form, rent: event.target.value })} data-testid="input-flat-rent" className={inputClass()} placeholder="25000" /></Field><Field label="Security deposit"><input min="0" type="number" value={form.deposit} onChange={(event) => setForm({ ...form, deposit: event.target.value })} data-testid="input-flat-deposit" className={inputClass()} placeholder="50000" /></Field></div><Field label="Tenure" hint="Move-in date to lease end date"><TenureField testId="button-tenure-range" startDate={form.moveInDate} endDate={form.tenureEnd} onChange={(moveInDate, tenureEnd) => setForm({ ...form, moveInDate, tenureEnd })} /></Field><Field label="Workplace"><input value={form.workplace} onChange={(event) => setForm({ ...form, workplace: event.target.value })} data-testid="input-tenant-workplace" className={inputClass()} placeholder="Optional" /></Field><Field label="Government ID" hint="Stored only for your records"><input value={form.govtId} onChange={(event) => setForm({ ...form, govtId: event.target.value })} data-testid="input-tenant-govt-id" className={inputClass()} placeholder="Optional ID reference" /></Field><div className="flex justify-end gap-2 pt-3"><button type="button" onClick={onClose} data-testid="button-cancel-flat" className="rounded-[8px] px-3.5 py-2.5 text-[12px] font-semibold text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]">Cancel</button><button type="submit" disabled={pending || !properties.length || !form.moveInDate || !form.tenureEnd} data-testid="button-submit-flat" className="rounded-[8px] bg-[hsl(var(--primary))] px-4 py-2.5 text-[12px] font-bold text-[hsl(var(--primary-foreground))] disabled:opacity-50">{pending ? 'Saving...' : editing ? 'Save changes' : 'Add unit'}</button></div></form></Modal>;
}

function ExpiredLeaseModal({ flat, form, setForm, onClose, onSubmitRenew, onDelete, renewPending }: { flat: Flat | null; form: { tenureEnd: string; rent: string }; setForm: (form: { tenureEnd: string; rent: string }) => void; onClose: () => void; onSubmitRenew: (event: FormEvent<HTMLFormElement>) => void; onDelete: () => void; renewPending: boolean }) {
  if (!flat) return null;
  return <Modal open={!!flat} title="Lease expired" description={`${flat.tenantName} · ${flat.flatNo} · tenure ended ${formatDate(flat.tenureEnd, true)}`} onClose={onClose}>
    <div className="space-y-5">
      <p className="text-[13px] leading-5 text-[hsl(var(--muted-foreground))]">This tenant's lease has ended. Renew with a new tenure end date (and rent, if it's changing), or remove them from the unit.</p>
      ...
      <button type="button" onClick={onDelete} data-testid="button-expired-delete" className="flex w-full items-center justify-center gap-1.5 rounded-[8px] border border-[hsl(var(--destructive)/.3)] py-2.5 text-[12px] font-bold text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive)/.08)]"><Trash2 size={14} />Delete tenant instead</button>
    </div>
  </Modal>;
}
