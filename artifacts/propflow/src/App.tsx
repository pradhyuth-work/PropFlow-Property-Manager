...
function AppShell() {
  ...
  const [flatForm, setFlatForm] = useState({ propertyId: '', flatNo: '', tenantName: '', workplace: '', govtId: '', moveInDate: '', deposit: '', rent: '' });
  ...
  const openCreateFlat = (propertyId = properties[0]?.id ?? '') => {
    setEditingFlat(null);
    setFlatForm({ propertyId, flatNo: '', tenantName: '', workplace: '', govtId: '', moveInDate: '', deposit: '', rent: '' });
    setFlatModal(true);
  };

  const openEditFlat = (flat: Flat) => {
    setEditingFlat(flat);
    setFlatForm({ propertyId: flat.propertyId, flatNo: flat.flatNo, tenantName: flat.tenantName, workplace: flat.workplace, govtId: flat.govtId, moveInDate: flat.moveInDate.slice(0, 10), deposit: String(flat.deposit), rent: String(flat.rent) });
    setFlatModal(true);
  };

  const submitFlat = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!flatForm.propertyId || !flatForm.flatNo.trim() || !flatForm.tenantName.trim() || !flatForm.rent) return;
    const flatData = { flatNo: flatForm.flatNo, tenantName: flatForm.tenantName, workplace: flatForm.workplace, govtId: flatForm.govtId, moveInDate: flatForm.moveInDate, deposit: Number(flatForm.deposit) || 0, rent: Number(flatForm.rent) || 0 };
    const onSuccess = () => {
      setFlatModal(false);
      refreshOverview();
  ...
  return (
    <div className="noise flex min-h-[100dvh] bg-[hsl(var(--background))]">
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-[248px] flex-col border-r border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar))] px-4 py-5 transition-transform duration-300 md:static md:translate-x-0 ${mobileNav ? 'translate-x-0' : '-translate-x-full'}`}>
...
function Metric({ label, value, detail, icon: Icon, accent, trend }: { label: string; value: string; detail: string; icon: typeof WalletCards; accent: string; trend?: 'up' | 'down' }) {
  return <Surface className="group relative overflow-hidden p-5 transition-transform duration-300 hover:-translate-y-1"><div className={`absolute right-0 top-0 h-20 w-20 rounded-bl-[48px] ${accent} opacity-[.13] transition-transform duration-300 group-hover:scale-125`} /><div className="mb-5 flex items-center justify-between"><span className="mono text-[10px] font-medium uppercase tracking-[.13em] text-[hsl(var(--muted-foreground))]">{label}</span><Icon size={17} className="text-[hsl(var(--muted-foreground))]" /></div><p data-testid={`text-metric-${label.toLowerCase().replaceAll(' ', '-')}`} className="display text-[26px] font-semibold tracking-[-.06em]">{value}</p><div className="mt-2 flex items-center gap-1.5 text-[11px] text-[hsl(var(--muted-foreground))]">{trend && <span className={trend === 'up' ? 'text-[hsl(var(--primary-foreground))]' : 'text-[hsl(var(--accent-foreground))]'}>{trend === 'up' ? <ArrowUpRight size={13} /> : <ArrowDownRight size={13} />}</span>}{detail}</div></Surface>;
}
